...
GetUARTByte
	banksel RX_temp
	movwf RX_temp ;сохраняем значение адреса в который надо сохранять
	call save_reg ;сохраняем регистры 
	banksel RX_temp
	movf RX_temp,0
	movwf FSR
	banksel RCREG	
	...
	bankisel RX_temp
	movwf INDF ;записываем считанный байт в очереднную ячейку буфера
	call load_reg;восстанавливаем сохраненные регистры
	return	
;------------------------------------------------------------------
...
UART_TX_OFF;выключаем передатчик
	...
	return
;-----------------------------------------------------------------------
load_reg ;восстанавливаем сохраненные регистры
	banksel STATUS_temp
	movf STATUS_temp,0 ;восстанавливаем STATUS
	movwf STATUS
	movf FSR_temp,0
	movwf FSR
	return
	;----------------------------------------------------
save_reg ;сохраняем регистры 
	banksel STATUS_temp
	movf STATUS,0
	movwf STATUS_temp ;сохраняем STATUS
	movf  FSR,0
	movwf FSR_temp		
	return
	;----------------------------------------------------
